        ...
        for (String c : Config.cfg.get("YouTube.channels").split(", ")) {
            if (checkIfOnline(c)) {
                if (!liveChannels.contains(c)) {
                    Bot.jda.getTextChannelById(Config.cfg.get("Notifications.channelID")).sendMessage((Language.lang.getProperty("YouTube.Live.WENT_LIVE")).replace("%CHANNEL%", getName()) + "  https://youtu.be/" + getId()).queue();
                    liveChannels.add(c);
                }
            } else {
