...
public class BlacklistListener extends ListenerAdapter {
    ...
    private static boolean canControlBot(Member member) {
        return member.getRoles().stream().anyMatch(r -> r.getName().equals(ConfigManager.getProperty("Bot controller role")));
    }

    public void onMessageReceived(MessageReceivedEvent event) {
        if (!ConfigManager.getConfig().getProperty("Link blacklist").equalsIgnoreCase("true") || event.getAuthor() == event.getJDA().getSelfUser() || event.getMessage().getAuthor().isBot() || !canControlBot(event.getMember()))
            return;
