...
public class BlacklistListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        if (permit.permitted.contains(event.getMember()) || !ConfigManager.getConfig().getProperty("Link blacklist").equalsIgnoreCase("true") || event.getAuthor() == event.getJDA().getSelfUser() || event.getMessage().getAuthor().isBot() || event.getMember().getRoles().stream().anyMatch(r -> r.getName().equals(ConfigManager.getProperty("Bot controller role"))))
            return;
