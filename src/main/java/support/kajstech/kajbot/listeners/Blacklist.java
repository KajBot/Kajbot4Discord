...
public class Blacklist extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        if (Permit.permitted.contains(event.getMember()) || !ConfigHandler.getProperty("Link blacklist").equalsIgnoreCase("true") || event.getAuthor() == event.getJDA().getSelfUser() || event.getMessage().getAuthor().isBot() || event.getMember().getRoles().stream().anyMatch(r -> r.getName().equals(ConfigHandler.getProperty("Bot controller role"))))
            return;
