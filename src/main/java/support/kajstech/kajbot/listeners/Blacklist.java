...
public class Blacklist extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        if (Permit.permitted.contains(event.getMember()) || !ConfigHandler.getProperty("Link blacklist").equalsIgnoreCase("true") || event.getAuthor() == event.getJDA().getSelfUser() || event.getMessage().getAuthor().isBot())
            return;
